...
By using the [wallet SDK](/references/wallets) alongside the [React SDK](/react), you can use smart wallets in your front-end applications easily.

<Steps>

<Step title="Deploy an Account Factory">

Deployable via the [explore page](https://thirdweb.com/explore/smart-wallet) or build your own [ERC 4337](https://eips.ethereum.org/EIPS/eip-4337) compatible factory contract using the [Solidity SDK](/solidity).
...
To use the [Connect Wallet](/references/react/ConnectWallet) component to connect your users to your app using smart wallet,
we need to pass a [`smartWallet`](/references/react/smartWallet) to the `supportedWallets` prop in [`ThirdwebProvider`](/references/react/ThirdwebProvider).

You can change the configuration based on your requirements,
but for this demo, we will enable **gasless transactions** and use a [EmbeddedWallet](/references/react/embeddedWallet) as the personal wallet.
...
The [useSmartWallet](/references/react/useSmartWallet) hook allows you to programmatically connect your application to the wallet. You will need to build your own UI for connecting the wallet.
...
Deploy a NFT Drop contract from the [explore page](https://thirdweb.com/explore/nft-drop) or build your own [ERC 721](https://eips.ethereum.org/EIPS/eip-721) compatible contract using the [Solidity SDK](/solidity).

use the dashboard to upload some NFTs that can be claimed with the smart wallet.

To claim an NFT, we will use the [useContract](/references/react/useContract) hook to fetch the contract and the [Web3Button](/react/react.web3button) component to send the transaction.
...
To add a session key to your smart wallet, you can use the [`useCreateSessionKey`](/references/react/useCreateSessionKey) hook to add a session key to the currently connected smart wallet:
